...
while(np.mean(roc_auc)<0.5):
    ...
    #oversampling the oddball stimuli to fix imabalance of the data
        ...
        for i in range(Xy.shape[0]):
            ...
            if Xy[i,-1]==0:
                temp=np.tile(Xy[i,:], (imbalance_rate,1)) # Xy_balanced[j:j+imbalance_rate,:]
                j=j+imbalance_rate
            else:
    ...
    #    AT_clfImage = svm.SVC(C=1000,probability=True).fit(data_X_train, data_y_train)
...
#meditationRest_analysisResults_5fold=dict(accuracy=accuracy,precision=precision,\
#                                    recall=recall,roc_auc=roc_auc,conf=conf,\
#                                    classifier=classifier)
#import pickle
#with open('meditationRest_analysisResults_5fold', 'wb') as fp:
#    pickle.dump(meditationRest_analysisResults_5fold, fp)
######    
